...
name: sdlc:ex-backup
description: >-
  [BACKUP — LEGACY] Bản sao lưu của sdlc-explore cũ (v2.0.2). Đã được thay thế bởi skill sdlc-explore mới khớp 100% chuẩn sdlc.
  Giữ lại để tham khảo. KHÔNG tự động kích hoạt — chỉ dùng khi cần tra cứu logic cũ.
...
| **Architect** | Load state → Pick service → Scout → Plan(tùy chọn) → Explore Pipeline (chỉ FR-Discovery+HLD) → Summary → Hỏi người dùng chọn tiếp | Chỉ cần kiến trúc |
...
Workflow xử lý: idempotent retry, FR discovery, gate verification, single-service LLD, FR distribution, IMP+TST.
...
- Pattern 2: HLD thất bại blocking → AskUserQuestion retry/skip/abort
...
Inputs: scout reports của service + libs, FR files (knowledge/04-microservices/{service}/), HLD (knowledge/03-system-architecture/), LLD (knowledge/04-microservices/{service}/tech-design.md), IMP+TST outputs (knowledge/04-microservices/{service}/).
