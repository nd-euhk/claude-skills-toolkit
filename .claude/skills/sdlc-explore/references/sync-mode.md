...
Update existing exploration documentation by detecting changes since last run, analyzing impact, and running only affected SDLC phases — for ONE service at a time.
...
Spawn `Agent(Explore)` để phân tích impact sâu hơn. Prompt:
...
Dựa trên SRS hiện tại ({srsPath}), HLD ({hldPath}), LLD ({lldPath}):
1. FR nào bị ảnh hưởng?
2. Service dependencies nào bị ảnh hưởng?
3. Architecture patterns nào thay đổi?
4. Đề xuất phases cần cập nhật (SRS/HLD/LLD/IMP/TST) và lý do.
...
- Question: "Phases nào cần chạy lại cho {service} dựa trên thay đổi?" (header: "Sync Scope")
  multiSelect: true
- Options:
  - "SRS" (pre-select nếu HIGH impact từ Tier 1+2)
...
function determineStartPhase(phases) {
  if (phases.includes('SRS')) return 'SRS'
...
### Libs thay đổi nhưng service không đổi
