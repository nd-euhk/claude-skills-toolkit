...
2. Activate `debugging` skill (systematic-debugging + root-cause-tracing).
...
Use `general-purpose` agent for external knowledge research.
...
Use `Plan` agent to create implementation plan. Fallback: `general-purpose` agent.
...
Implement per plan. Use `sequential-thinking`, `problem-solving` skills.
...
- Activate `sprint` for task sync-back, plan status updates, and progress tracking
...
|------|------------------|
| 1 | parallel `Explore` subagents |
| 2 | `debugging`, `sequential-thinking`, parallel `Explore`, (`problem-solving` auto) |
| 3 | `general-purpose` (runs parallel with steps 1+2) |
| 4 | `brainstormer` |
| 5 | `problem-solving`, `sequential-thinking` |
| 6 | `tester`, parallel `Bash` verification |
| 7 | `code-reviewer` |
