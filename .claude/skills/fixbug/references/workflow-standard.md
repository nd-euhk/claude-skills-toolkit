...
- Activate `sprint` for task sync-back and plan status updates
- Ask to commit via `git-manager` subagent
- Spawn `journal-writer` subagent
...
| 2 | `debugging`, `sequential-thinking`, `debugger` subagent, parallel `Explore`, (`problem-solving` auto) |
