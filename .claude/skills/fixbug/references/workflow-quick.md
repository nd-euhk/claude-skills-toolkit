...
Activate `debugging` skill. Activate `sequential-thinking` for structured analysis.
...
1. **Activate `sprint` skill (MANDATORY)** → sync plan/task status if fix is part of a plan, update progress, hydrate Claude Tasks.
2. `TaskUpdate` to mark Claude Tasks complete.
3. Spawn `git-manager` subagent to commit.
4. Spawn `journal-writer` subagent to log decisions.
...
|------|------------------|
| 1 | Launch Agent(Explore) (minimal) or direct file read |
| 2 | `debugging`, `sequential-thinking` |
| 3 | Parallel `Bash` for verification |
| 4 | `code-reviewer` subagent |
| 5 | Report |
| 6 | `sprint` (MANDATORY), `git-manager`, `journal-writer` |
...
- Step 6 finalize is MANDATORY for every fix — `sprint` is NOT optional
- Specialized subagents (`code-reviewer`, `journal-writer`): fall back to general-purpose agents with explicit review/log prompts if unavailable
