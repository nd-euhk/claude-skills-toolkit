...
Use `Explore` subagents to find issue locations (previously `ck:scout`).
...
EnterPlanMode and use Agent(Plan) agent to write then ExitPlanMode tool, fallback Use `general-purpose` agent.
