...
name: fixbug
description: >-
  Fix bugs, errors, test failures, and CI/CD issues with intelligent routing.
...
allowed-tools: Read,Write,Edit,Bash,TaskCreate,TaskUpdate,TaskGet,TaskList,Agent,AskUserQuestion,WebFetch,WebSearch
---
...
Detect these flags in the user's message to skip Step 0 mode selection:

| Flag | Mode | Behavior |
|------|------|----------|
| `--auto` | Autonomous (**default**) | Auto-approve when quality gates pass; high-risk fixes stop for human approval |
...
**This diagram is the authoritative workflow.** All workflow variants (Quick, Standard, Deep) follow this core flow. Deep workflow expands Step 3 (Complexity Assessment) into parallel Research + Brainstorm + Plan phases (9 task phases), while Quick workflow merges Fix+Verify into a single step. If prose conflicts with this flow, follow the diagram.

## Workflow

### Step 0: Mode Selection

**First action:** Check user's message for mode flags (`--auto`, `--quick`, `--review`, `--parallel`). If a flag is detected, skip to that mode directly.

**If no flag detected**, use `AskUserQuestion` to determine workflow mode:
    ...
    "options": [
      {
        ...
        "description": "Auto-approve if quality high, only ask when stuck or high-risk"
      },
      {
      ...
      {
        "label": "Quick fix",
        "description": "Fast debug-fix-review cycle for simple issues (type errors, lint)"
      }
    ],
    "multiSelect": false
  }]
...
**Mode recommendations by issue type:**
...
**Skip mode selection when:** issue is clearly trivial (type error keyword detected → default Quick), user specified mode flag, or previous context already established mode.
...
| Level | Indicators | Workflow | Step Mapping |
|-------|------------|----------|--------------|
| **Simple** | Single file, clear error, type/lint | `references/workflow-quick.md` | Core steps 0-6 (merges Fix+Verify) |
| **Moderate** | Multi-file, root cause unclear | `references/workflow-standard.md` | Core steps 0-6 (1:1 mapping) |
| **Complex** | System-wide, architecture impact | `references/workflow-deep.md` | Expands Step 3 → 3 parallel phases (9 total) |
| **Parallel** | 2+ independent issues OR `--parallel` flag | Per-issue task trees | Each issue follows its own complexity workflow |
...
5. **Artifact gate:** Write review artifacts to .work/bugs/{BUG-ID}/ (context-snippets.json, risk-gate.json, verification.json, review-decision.json).
...
- `sprint` — MANDATORY for sync-back and progress tracking, every fix

**Conditional:**
- `problem-solving` — auto-triggers when 2+ hypotheses fail in Step 2
- `brainstormer` agent — multiple valid approaches, architecture decision (Deep only)

**Subagents:** `debugger`, `code-reviewer`, `tester`, `Bash`
