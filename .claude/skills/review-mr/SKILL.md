...
description: >-
  ...
  security, bugs, and CLAUDE.md conventions. Use when reviewing MR/PR, review merge
  request, check MR, audit pull request, code review before merge, evaluate merge
  request changes, or run pre-merge review. Supports --arch, --security, --bugs,
  --conventions, --full flags.
version: 1.0.0
allowed-tools:
  - Read
  - Write
...
Deep review of GitLab/GitHub merge requests across 4 independent dimensions, each handled by a specialized subagent running in parallel.
...
```bash
# Full review (all 4 dimensions in parallel)
...
- **Dimension flags**: `--arch`, `--security`, `--bugs`, `--conventions`, `--full`
...
1. "Full Review (Architecture + Security + Bugs + Conventions)" — runs all 4 subagents
2. "Architecture Review only" — C4 model, ADR, patterns, impact analysis
3. "Security Review only" — OWASP Top 10, secrets, auth, data exposure
4. "Bug Detection only" — Logic, race conditions, edge cases, error handling
...
- Any CRITICAL (security) or URGENT (arch) → **URGENT**
- Any BUG_FOUND (bugs) or VIOLATION (conventions) → **NEEDS_ATTENTION**
- Otherwise → **APPROVED**
...
- **Options**:
  ...
  2. "Chỉ post CRITICAL + URGENT" — only highest severity
...
/review-mr --full gl-456
  → Detect platform from current repo → GitLab MR #456 → all 4 subagents in parallel
...
- **URGENT beats everything** — if architecture says URGENT or security says CRITICAL, the overall verdict is URGENT.
...
- **Token efficiency** — main SKILL.md is ~250 lines. Detailed workflows live in `references/` and load on-demand.
...
- `references/workflow-full.md` — Full review orchestration: 4-subagent parallel dispatch, synthesis, deduplication
