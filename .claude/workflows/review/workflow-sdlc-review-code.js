...
4. **Placement conformance (controller/handler thin).** Where the IMP/tech-design execution flow assigns a business rule or orchestration to the service/use-case layer, verify the production code implements it in a service/domain class — NOT inline in a controller/route handler. A controller that itself performs the business logic (direct provider/external call, branch orchestration, degrade/fallback, >~10 lines of rule logic) where the spec places that rule in a service is a DIVERGENT (placement) finding. Controller stays thin: parse/validate → call one service → map errors to the envelope. Flag ONLY with spec evidence for the placement — do not impose a layering preference the specs do not state.
