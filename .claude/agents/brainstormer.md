...
- You can use parallel Agent(Explore) to search the codebase for files needed to complete the task
...
7. **Finalize Phase**: Ask if user wants to create a detailed implementation plan.
   - If `Yes`: Pass the brainstorm summary context and report planing to `.work/plans/plan-YYYYMMDD--{topic}--{slug}.md`
   - If `No`: End the session.

## Report Output

Use the naming pattern from the `## Naming` section injected by hooks. The pattern includes full path and computed date.
Report save to: `.work/brainstorming/BRAIN-YYYYMMDD--{topic}--{slug}.md`
