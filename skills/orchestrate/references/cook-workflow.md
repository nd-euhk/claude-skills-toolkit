...
Direct TDD implementation following RED → GREEN → REFACTOR cycle. **Only pick ✅ Ready tasks from the board.** Never implement 🔲 Todo or 🚧 In Progress tasks.

## Pre-Flight Check

### Step 1: Get Ready Tasks from Board

Use sprint skill to find tasks ready for implementation:

```
Skill: sprint
Description: "Get all ✅ Ready tasks from .work/board.md. List them with task IDs, feature IDs, descriptions, and assigned services."
```

If no ✅ Ready tasks exist:
- Tell user: "No Ready tasks on the board. Run New Feature or Change Request workflow first to create specs and mark tasks as Ready."
- STOP — do not proceed.

If Ready tasks exist, present them to the user and let them choose which to implement.

### Step 2: Mark Task as In Progress

Before starting TDD, use sprint skill to mark the selected task:

```
Skill: sprint
Description: "Update board task <T-XXX> status from ✅ Ready to 🚧 In Progress."
```

### Step 3: Verify Spec Readiness
...
After all TDD cycles pass:

1. **Update board task to Done via sprint skill:**

```
Skill: sprint
Description: "Update board task <T-XXX> status from 🚧 In Progress to ✅ Done. 
Feature <FR-ID> implementation is complete, all tests pass."
```

2. **Run sprint reverse sync** to cascade status upstream:

```
Skill: sprint
Description: "Run reverse sync: check if all board tasks for <BL-XXX> are Done → update backlog.
Then check if all features in the phase are Done → update roadmap."
```
