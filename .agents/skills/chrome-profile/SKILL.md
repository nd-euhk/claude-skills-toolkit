...
This is not the default for ordinary browser testing. Use `agent-browser` when a fresh or tool-managed browser is enough. Use project-native Playwright for repeatable CI tests. Use this skill when the user's actual Chrome profile state matters.
...
Use `chrome-profile setup --yes` for non-interactive bootstrap.
