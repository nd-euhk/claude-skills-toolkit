...
digraph stuck_dispatch {
    ...
    debug [label="skills/debugging/\references/\nsystematic-debugging.md", shape=box, style="rounded,filled", fillcolor=lightyellow];
}
```
...
| **Code broken** - Wrong behavior, test failing, unexpected output | skills/debugging/references/systematic-debugging.md |
| **Multiple independent problems** - Can parallelize investigation | Spawn parallel Agent(Explore) |
| **Root cause unknown** - Symptom clear, cause hidden | skills/debugging/references/root-cause-tracing.md |
